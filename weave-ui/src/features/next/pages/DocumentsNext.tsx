...
// status → card-language tone + label (mirrors the classic colored labels)
const STONE: Record<string, string> = {
  processed: 'good', preprocessed: 'comm', processing: 'accent', pending: 'warn', failed: 'crit'
}
const SLABEL: Record<string, string> = {
  processed: 'Completed', preprocessed: 'Preprocessed', processing: 'Processing',
...
export default function DocumentsNext() {
  ...
  return (
    <div className="cgdocs">
      <div className="cgdocs-top">
        <div>
          <div className="eyebrow">Knowledge · Documents</div>
          <h1>Documents</h1>
        </div>
        <div className="actions">
          <button className="btn ghost" onClick={scanDocuments} disabled={isRefreshing}>
            <RefreshCwIcon className="size-4" /> Scan
          </button>
          <button
            className={cn('btn ghost', pipelineBusy && 'pipeline-busy')}
            onClick={() => setShowPipelineStatus(true)}
            title={t('documentPanel.documentManager.pipelineStatusTooltip')}
          >
            <ActivityIcon className="size-4" /> Pipeline
          </button>
          {isSelectionMode && (
            <DeleteDocumentsDialog selectedDocIds={selectedDocIds} onDocumentsDeleted={handleDocumentsDeleted} />
          )}
          {isSelectionMode && hasCurrentPageSelection ? (
            (() => {
              const b = getSelectionButtonProps()
              const Icon = b.icon
              return (
                <button className="btn ghost" onClick={b.action} title={b.text}>
      ...
      <div className="cgdocs-toolbar">
        <div className="filterchips">
          {([
            { key: 'all', label: 'All', tone: 'all', count: statusCounts.all || documentCounts.all },
            { key: 'processed', label: 'Completed', tone: 'good', count: processedCount },
            { key: 'preprocessed', label: 'Preprocessed', tone: 'comm', count: preprocessedCount },
            { key: 'processing', label: 'Processing', tone: 'accent', count: processingCount },
            { key: 'pending', label: 'Pending', tone: 'warn', count: pendingCount },
            { key: 'failed', label: 'Failed', tone: 'crit', count: failedCount }
          ] as { key: StatusFilter; label: string; tone: string; count: number }[]).map((f) => (
            <button
              key={f.key}
              data-tone={f.tone}
              className={cn('fchip', statusFilter === f.key && 'active')}
        ...
        <button className="btn sm ghost" onClick={handleManualRefresh} disabled={isRefreshing}
          title={t('documentPanel.documentManager.refreshTooltip')}>
          <RotateCcwIcon className="size-4" />
        </button>
        <button className="btn sm ghost" onClick={() => setShowFileName(!showFileName)}>
          {showFileName ? 'Show ID' : 'Show path'}
        </button>
      ...
      <div className="cgdocs-table" ref={cardContentRef}>
        {!docs || (filteredAndSortedDocs && filteredAndSortedDocs.length === 0) ? (
          <div className="empty" style={{ padding: 44 }}>
            {docs
              ? 'No documents match this filter.'
              : 'No documents yet — use Upload or Scan to add content.'}
          </div>
        ) : (
          <table className="cgtable">
            <thead>
              <tr>
                <th className="sortable" onClick={() => handleSort(showFileName ? 'file_path' : 'id')}>
                  {showFileName ? 'File' : 'ID'}
                ...
                <th className="sortable" onClick={() => handleSort('created_at')}>
                  Created{' '}
                  {sortField === 'created_at' && (sortDirection === 'asc' ? <ArrowUpIcon size={12} /> : <ArrowDownIcon size={12} />)}
                </th>
                <th className="sortable" onClick={() => handleSort('updated_at')}>
                  Updated{' '}
                  {sortField === 'updated_at' && (sortDirection === 'asc' ? <ArrowUpIcon size={12} /> : <ArrowDownIcon size={12} />)}
                </th>
                <th style={{ width: 48, textAlign: 'center' }}>Sel</th>
              </tr>
            </thead>
            <tbody>
              {filteredAndSortedDocs &&
                filteredAndSortedDocs.map((doc) => {
                  const tone = STONE[doc.status] || 'accent'
                  const label = SLABEL[doc.status] || doc.status
                  const hasMeta = !!(doc.metadata && Object.keys(doc.metadata).length > 0)
                  const tip = [
                    doc.track_id ? `Track ID: ${doc.track_id}` : '',
                    hasMeta ? formatMetadata(doc.metadata!) : '',
                    doc.error_msg || ''
                  ].filter(Boolean).join('\n')
                  return (
                    <tr key={doc.id}>
                      <td>
                        {showFileName ? (
                          <>
                            <div className="fp" title={doc.file_path}>{getDisplayFileName(doc, 40)}</div>
                            <div className="subid">{doc.id}</div>
                          </>
                        ) : (
                          <div className="fp" title={doc.file_path}>{doc.id}</div>
                        )}
                      </td>
                      <td><div className="sum" title={doc.content_summary}>{doc.content_summary}</div></td>
                      <td>
                        <span className={cn('st', tone)} title={tip || undefined}>
                          <span className="dot" />
                          {label}
                          {doc.error_msg ? (
                      ...
                      <td className="num">{doc.content_length ?? '—'}</td>
                      <td className="num">{doc.chunks_count ?? '—'}</td>
                      <td className="when">{new Date(doc.created_at).toLocaleString()}</td>
                      <td className="when">{new Date(doc.updated_at).toLocaleString()}</td>
