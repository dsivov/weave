...
import { useFlowStore, type FlowNodeData } from '@/features/next/diagram-editor/lib/store'
...
function CanvasInner({ onOpenPalette }: CanvasInnerProps) {
  ...
  // Typed from React Flow's own OnNodeDrag rather than spelling the event out:
  // `MouseEvent` here is React's synthetic one (imported for the div handlers
  // below), but React Flow hands drag callbacks the DOM event.
  const handleNodeDragStop = useCallback<OnNodeDrag<Node<FlowNodeData>>>(
    (_event, draggedNode) => {
      pushHistory()
