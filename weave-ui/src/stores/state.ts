...
import { checkHealth, WeaveStatus } from '@/api/weave'
...
const initAuthState = (): { isAuthenticated: boolean; isGuestMode: boolean; coreVersion: string | null; apiVersion: string | null; username: string | null; webuiTitle: string | null; webuiDescription: string | null; lastTokenRenewal: string | null; tokenExpiresAt: number | null } => {
  ...
  const webuiTitle = localStorage.getItem('WEAVE-WEBUI-TITLE');
...
export const useAuthStore = create<AuthState>(set => {
  ...
  return {
    ...
    login: (token, isGuest = false, coreVersion = null, apiVersion = null, webuiTitle = null, webuiDescription = null) => {
      localStorage.setItem('WEAVE-API-TOKEN', token);

      if (coreVersion) {
        localStorage.setItem('WEAVE-CORE-VERSION', coreVersion);
      }
      if (apiVersion) {
        localStorage.setItem('WEAVE-API-VERSION', apiVersion);
      }

      if (webuiTitle) {
        localStorage.setItem('WEAVE-WEBUI-TITLE', webuiTitle);
      } else {
        localStorage.removeItem('WEAVE-WEBUI-TITLE');
      }

      if (webuiDescription) {
        localStorage.setItem('WEAVE-WEBUI-DESCRIPTION', webuiDescription);
      } else {
        localStorage.removeItem('WEAVE-WEBUI-DESCRIPTION');
      }
      ...
      localStorage.setItem('WEAVE-LAST-TOKEN-RENEWAL', formattedTime);
    ...
    logout: () => {
      localStorage.removeItem('WEAVE-API-TOKEN');
      localStorage.removeItem('WEAVE-LAST-TOKEN-RENEWAL');
