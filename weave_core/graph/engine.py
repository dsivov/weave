...
from weave_core.constants import GRAPH_FIELD_SEP
...
from weave_core.types import KnowledgeGraph
...
@dataclass
class WeaveEngine:
    """WeaveEngine: Simple and Fast Retrieval-Augmented Generation."""
    ...
    top_k: int = field(default=get_env_value("WEAVE_TOP_K", DEFAULT_TOP_K, int))
    """Number of entities/relations to retrieve for each query."""

    chunk_top_k: int = field(
        default=get_env_value("WEAVE_CHUNK_TOP_K", DEFAULT_CHUNK_TOP_K, int)
    )
    """Maximum number of chunks in context."""

    max_entity_tokens: int = field(
        default=get_env_value("WEAVE_MAX_ENTITY_TOKENS", DEFAULT_MAX_ENTITY_TOKENS, int)
    )
    """Maximum number of tokens for entity in context."""

    max_relation_tokens: int = field(
        default=get_env_value("WEAVE_MAX_RELATION_TOKENS", DEFAULT_MAX_RELATION_TOKENS, int)
    )
    """Maximum number of tokens for relation in context."""

    max_total_tokens: int = field(
        default=get_env_value("WEAVE_MAX_TOTAL_TOKENS", DEFAULT_MAX_TOTAL_TOKENS, int)
    )
    """Maximum total tokens in context (including system prompt, entities, relations and chunks)."""

    cosine_threshold: int = field(
        default=get_env_value("WEAVE_COSINE_THRESHOLD", DEFAULT_COSINE_THRESHOLD, int)
    )
    """Cosine threshold of vector DB retrieval for entities, relations and chunks."""

    related_chunk_number: int = field(
        default=get_env_value("WEAVE_RELATED_CHUNK_NUMBER", DEFAULT_RELATED_CHUNK_NUMBER, int)
    )
    """Number of related chunks to grab from single entity or relation."""

    kg_chunk_pick_method: str = field(
        default=get_env_value("WEAVE_KG_CHUNK_PICK_METHOD", DEFAULT_KG_CHUNK_PICK_METHOD, str)
    )
    ...
    entity_extract_max_gleaning: int = field(
        default=get_env_value("WEAVE_MAX_GLEANING", DEFAULT_MAX_GLEANING, int)
    )
    """Maximum number of entity extraction attempts for ambiguous content."""

    max_extract_input_tokens: int = field(
        default=get_env_value(
            "WEAVE_MAX_EXTRACT_INPUT_TOKENS", DEFAULT_MAX_EXTRACT_INPUT_TOKENS, int
        )
    )
    ...
    chunk_token_size: int = field(default=int(os.getenv("WEAVE_CHUNK_SIZE", 1200)))
    """Maximum number of tokens per text chunk when splitting documents."""

    chunk_overlap_token_size: int = field(
        default=int(os.getenv("WEAVE_CHUNK_OVERLAP_SIZE", 100))
    )
    ...
    embedding_batch_num: int = field(default=int(os.getenv("WEAVE_EMBEDDING_BATCH_NUM", 10)))
    """Batch size for embedding computations."""

    embedding_func_max_async: int = field(
        default=int(os.getenv("WEAVE_EMBEDDING_FUNC_MAX_ASYNC", 8))
    )
    ...
    default_embedding_timeout: int = field(
        default=int(os.getenv("WEAVE_EMBEDDING_TIMEOUT", DEFAULT_EMBEDDING_TIMEOUT))
    )
    ...
    summary_max_tokens: int = field(
        default=int(os.getenv("WEAVE_SUMMARY_MAX_TOKENS", DEFAULT_SUMMARY_MAX_TOKENS))
    )
    """Maximum tokens allowed for entity/relation description."""

    summary_context_size: int = field(
        default=int(os.getenv("WEAVE_SUMMARY_CONTEXT_SIZE", DEFAULT_SUMMARY_CONTEXT_SIZE))
    )
    """Maximum number of tokens allowed per LLM response."""

    summary_length_recommended: int = field(
        default=int(
            os.getenv("WEAVE_SUMMARY_LENGTH_RECOMMENDED", DEFAULT_SUMMARY_LENGTH_RECOMMENDED)
        )
    )
    """Recommended length of LLM summary output."""

    llm_model_max_async: int = field(
        default=int(os.getenv("WEAVE_MAX_ASYNC", DEFAULT_MAX_ASYNC))
    )
    ...
    default_llm_timeout: int = field(
        default=int(os.getenv("WEAVE_LLM_TIMEOUT", DEFAULT_LLM_TIMEOUT))
    )
    ...
    min_rerank_score: float = field(
        default=get_env_value("WEAVE_MIN_RERANK_SCORE", DEFAULT_MIN_RERANK_SCORE, float)
    )
    ...
    max_parallel_insert: int = field(
        default=int(os.getenv("WEAVE_MAX_PARALLEL_INSERT", DEFAULT_MAX_PARALLEL_INSERT))
    )
    """Maximum number of parallel insert operations."""

    max_graph_nodes: int = field(
        default=get_env_value("WEAVE_MAX_GRAPH_NODES", DEFAULT_MAX_GRAPH_NODES, int)
    )
    """Maximum number of graph nodes to return in knowledge graph queries."""

    max_source_ids_per_entity: int = field(
        default=get_env_value(
            "WEAVE_MAX_SOURCE_IDS_PER_ENTITY", DEFAULT_MAX_SOURCE_IDS_PER_ENTITY, int
        )
    )
    ...
    max_source_ids_per_relation: int = field(
        default=get_env_value(
            "WEAVE_MAX_SOURCE_IDS_PER_RELATION",
            DEFAULT_MAX_SOURCE_IDS_PER_RELATION,
            int,
        )
    )
    ...
    max_file_paths: int = field(
        default=get_env_value("WEAVE_MAX_FILE_PATHS", DEFAULT_MAX_FILE_PATHS, int)
    )
    ...
    addon_params: dict[str, Any] = field(
        default_factory=lambda: {
            "language": get_env_value(
                "WEAVE_SUMMARY_LANGUAGE", DEFAULT_SUMMARY_LANGUAGE, str
            ),
            "entity_types": get_env_value("WEAVE_ENTITY_TYPES", DEFAULT_ENTITY_TYPES, list),
        }
    )
    ...
    # TODO: Deprecated (WeaveEngine will never initialize storage automatically on creation，and finalize should be call before destroying)
    auto_manage_storages_states: bool = field(default=False)
    """If True, weave_core will automatically calls initialize_storages and finalize_storages at the appropriate times."""

    cosine_better_than_threshold: float = field(
        default=float(os.getenv("WEAVE_COSINE_THRESHOLD", 0.2))
    )
    ...
    def __post_init__(self):
        ...
        logger.debug(f"WeaveEngine init with param:\n  {_print_config}\n")
        ...
        # while preventing side effects when the same EmbeddingFunc is reused across multiple WeaveEngine instances
    ...
    async def initialize_storages(self):
        ...
        if self._storages_status == StoragesStatus.CREATED:
            ...
            if default_workspace is None:
                ...
                logger.info(
                    f"Creating WeaveEngine instance with workspace='{self.workspace}' "
                    f"while default workspace is set to '{default_workspace}'"
                )

            # Auto-initialize pipeline_status for this workspace
            from weave_core.store.locks import initialize_pipeline_status
    ...
    async def adelete_by_entity(self, entity_name: str) -> DeletionResult:
        ...
        Returns:
            DeletionResult: An object containing the outcome of the deletion process.
        """
        from weave_core.graph.query import adelete_by_entity
    ...
    async def adelete_by_relation(
        ...
        Returns:
            DeletionResult: An object containing the outcome of the deletion process.
        """
        from weave_core.graph.query import adelete_by_relation
    ...
    async def get_entity_info(
        ...
        from weave_core.graph.query import get_entity_info
    ...
    async def get_relation_info(
        ...
        from weave_core.graph.query import get_relation_info
    ...
    async def aedit_entity(
        ...
        """
        from weave_core.graph.query import aedit_entity
    ...
    async def aedit_relation(
        ...
        """
        from weave_core.graph.query import aedit_relation
    ...
    async def acreate_relation(
        ...
        """
        from weave_core.graph.query import acreate_relation
    ...
    async def amerge_entities(
        ...
        """
        from weave_core.graph.query import amerge_entities
    ...
    async def aexport_data(
        ...
        """
        from weave_core.utils import aexport_data as utils_aexport_data
