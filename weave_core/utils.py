...
def _patch_ascii_colors_console_handler() -> None:
    ...
    if getattr(ConsoleHandler, "_weave_patched", False):
        return
    ...
    ConsoleHandler._weave_patched = True  # type: ignore[attr-defined]
...
if TYPE_CHECKING:
    from weave_core.graph.base import BaseKVStorage, BaseVectorStorage, QueryParam
...
VERBOSE_DEBUG = os.getenv("WEAVE_VERBOSE", "false").lower() == "true"
...
class WeavePathFilter(logging.Filter):
    """Filter for weave_core logger to filter out frequent path access logs"""
...
def setup_logger(
    ...
    if enable_file_logging:
        ...
        log_backup_count = get_env_value(
            "WEAVE_LOG_BACKUP_COUNT", DEFAULT_LOG_BACKUP_COUNT, int
        )
...
class EmbeddingFunc:
    ...
    Args:
        ...
        max_token_size: Enable embedding token limit checking for description summarization(Set embedding_token_limit in WeaveEngine)
...
async def invalidate_query_cache(hashing_kv) -> int:
    ...
    try:
        ...
        if keys_to_remove:
            from weave_core.store.locks import set_all_update_flags
...
async def aexport_data(
    ...
    if file_format == "csv":
        ...
        with open(output_path, "w", encoding="utf-8") as mdfile:
            mdfile.write("# WeaveEngine Data Export\n\n")
        ...
        with open(output_path, "w", encoding="utf-8") as txtfile:
            txtfile.write("WEAVE DATA EXPORT\n")
            txtfile.write("=" * 80 + "\n\n")
...
async def apply_rerank_if_enabled(
    ...
    try:
        ...
        rerank_max_chars = int(os.environ.get("WEAVE_RERANK_MAX_TOKENS_PER_DOC", "4096"))
        document_texts = []
