...
class StorageNotInitializedError(RuntimeError):
    ...
    def __init__(self, storage_type: str = "Storage"):
        super().__init__(
            ...
            f"\n"
            f"See: https://github.com/HKUDS/WeaveEngine#important-initialization-requirements"
        )
...
class PipelineNotInitializedError(KeyError):
    ...
    def __init__(self, namespace: str = ""):
        msg = (
            ...
            f"  2. For multi-workspace setups, each WeaveEngine instance was properly initialized\n"
            f"\n"
            f"Standard initialization:\n"
            f"  rag = WeaveEngine(workspace='your_workspace')\n"
            f"  await rag.initialize_storages()  # Auto-initializes pipeline_status\n"
            f"\n"
            f"If you need manual control (advanced):\n"
            f"  from weave_core.store.locks import initialize_pipeline_status\n"
