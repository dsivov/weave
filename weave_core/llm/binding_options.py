...
from weave_core.constants import DEFAULT_TEMPERATURE
...
# in WeaveEngine. It provides a standardized framework for:
#
...
class BindingOptions:
    ...
    def args_env_name_type_value(cls):
        ...
        # WEAVE_-prefixed like every other variable Weave itself reads (D-024).
        # These configure *our* calls into a connector; the vendor SDK never
        # reads them, so the prefix is safe and the config surface stays one
        # namespace instead of two.
        env_var_prefix = f"WEAVE_{cls._binding_name}_".upper()
        help = cls._help
    ...
    def generate_dot_env_sample(cls):
        """
        Generate a sample .env file for all WeaveEngine binding options.
        ...
        sample_top = (
            ...
            + "\n"
            + (
                "# Autogenerated .env entries list for WeaveEngine binding options\n"
                "#\n"
                "# To generate run:\n"
                "# $ python -m weave_core.llm.binding_options\n"
            )
            + "#" * 80
            + "\n"
        )
        ...
        sample_bottom = (
            ("#\n# End of .env entries for WeaveEngine binding options\n")
            + "#" * 80
            + "\n"
        )
...
#
# Usage:
#   python -m weave_core.llm.binding_options           # Generate .env sample
#   python -m weave_core.llm.binding_options test      # Test argument parsing
#
# =============================================================================
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        ...
        os.environ["WEAVE_OPENAI_LLM_REASONING"] = (
        ...
        try:
            ...
            if "WEAVE_OPENAI_LLM_REASONING" in os.environ:
                del os.environ["WEAVE_OPENAI_LLM_REASONING"]
