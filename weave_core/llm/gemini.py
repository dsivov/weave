"""
Gemini LLM binding for WeaveEngine.

This module provides asynchronous helpers that adapt Google's Gemini models
to the same interface used by the rest of the WeaveEngine LLM bindings. The
...
def _ensure_api_key(api_key: str | None) -> str:
    ...
    key = api_key or os.getenv("WEAVE_LLM_BINDING_API_KEY") or os.getenv("GEMINI_API_KEY")
    if not key:
        raise ValueError(
