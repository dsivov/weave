from weave_core.utils import verbose_debug, VERBOSE_DEBUG
import os
import logging
...
from weave_core.version import __api_version__
...
def create_openai_async_client(
    ...
    if use_azure:
        ...
        default_headers = {
            "User-Agent": f"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_8) WeaveEngine/{__api_version__}",
        ...
        if base_url is not None:
            ...
            merged_configs["base_url"] = os.environ.get(
                "WEAVE_OPENAI_API_BASE", "https://api.openai.com/v1"
            )
...
async def azure_openai_complete_if_cache(
    ...
    **kwargs,
    ...
    deployment = os.getenv("WEAVE_AZURE_OPENAI_DEPLOYMENT") or model or os.getenv("WEAVE_LLM_MODEL")
    base_url = (
        base_url or os.getenv("AZURE_OPENAI_ENDPOINT") or os.getenv("WEAVE_LLM_BINDING_HOST")
    )
    api_key = (
        api_key or os.getenv("AZURE_OPENAI_API_KEY") or os.getenv("WEAVE_LLM_BINDING_API_KEY")
    )
    api_version = (
        api_version
...
async def azure_openai_complete(
    ...
    **kwargs,
    ...
    result = await azure_openai_complete_if_cache(
        os.getenv("WEAVE_LLM_MODEL", "gpt-4o-mini"),
        prompt,
        system_prompt=system_prompt,
        history_messages=history_messages,
        keyword_extraction=keyword_extraction,
...
async def azure_openai_embed(
    ...
    # Handle Azure-specific environment variables and parameters
    deployment = (
        os.getenv("WEAVE_AZURE_EMBEDDING_DEPLOYMENT")
        or model
        or os.getenv("WEAVE_EMBEDDING_MODEL", "text-embedding-3-small")
    )
    base_url = (
        base_url
        or os.getenv("WEAVE_AZURE_EMBEDDING_ENDPOINT")
        or os.getenv("WEAVE_EMBEDDING_BINDING_HOST")
    )
    api_key = (
        api_key
        or os.getenv("WEAVE_AZURE_EMBEDDING_API_KEY")
        or os.getenv("WEAVE_EMBEDDING_BINDING_API_KEY")
    )
    api_version = (
        api_version
        or os.getenv("WEAVE_AZURE_EMBEDDING_API_VERSION")
        or os.getenv("AZURE_OPENAI_API_VERSION")
        or os.getenv("OPENAI_API_VERSION")
        or "2024-08-01-preview"
    )
