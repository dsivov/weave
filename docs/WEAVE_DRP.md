...
| # | Requirement | Priority | Rationale |
|---|-------------|:--------:|-----------|
| R19 | `Feature`, `Review`, `Insight` and `Question` exist as object types with link types `implemented_by`, `specified_by`, `depicted_by`, `answered_by` — **plus `produced` (Task→Commit) and `yielded` (Review→Insight)**, both from this document's class diagram and both required by a canonical traversal. `depicted_by` is a **derived view** over `Diagram.depicts`, never a stored edge (D-031). | must | D-011, D-031. Functionality has no node today; reviews and learnings are list fields on a task record and cannot be traversed or cited. The two extra links are not scope creep: `implemented_by` narrowed to Feature→Task, so the task-to-commit leg needed its own name, and `/ask/learnings` cannot reach an `Insight` without `yielded`. |
