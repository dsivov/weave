...
- [x] `weave_core/graph/storage/postgres.py` — `WEAVE_VDB_DECISIONS` and `WEAVE_VDB_COMMUNITIES`: DDL, `NAMESPACE_TABLE_MAP` entries, `upsert` branches, query SQL, index handling, and migration entries alongside the existing three.
  - **The task list above named three places and there were four.** `NAMESPACE_TABLE_MAP`, `PGVectorStorage.upsert`'s dispatch (which ends in `raise ValueError(f"{self.namespace} is not supported")`) and `PGVectorStorage.query`'s `SQL_TEMPLATES[self.namespace]` lookup were the three. The fourth was **`PostgreSQLDB.check_tables`** — a *different class*, reached at **connect** time — carrying a **hardcoded set of the three vector tables to skip**, because only `setup_table()` knows the embedding dimension to substitute into `VECTOR(dimension)`. A fourth vector table was not on that list, so startup ran the template verbatim: `asyncpg.exceptions.InvalidTextRepresentationError: invalid input syntax for type integer: "dimension"`.
  - **Fixed by deriving the set, not extending it:** skip any table whose DDL contains the `VECTOR(dimension)` placeholder. The placeholder is the evidence that the table needs a dimension, so the check asks the DDL rather than remembering a list — and the *next* vector store is handled by code nobody edits. Asserted as a derivation in `tests/test_postgres_runs_quadruple_mode.py`.
  - **Found only by running it.** The two builders were also defined on `PGKVStorage` rather than `PGVectorStorage` — the module imported cleanly, ruff was silent, and the symptom was an `AttributeError` on the first upsert. Both misses are the same shape: *a static check cannot tell you which class a method landed on, or which list a name is missing from.*
- [x] `tests/test_postgres_runs_quadruple_mode.py` — the two namespaces round-trip on **live PostgreSQL**: upsert, query by vector, delete. `weave` on 5442, `vector 0.8.5`; marked `integration`, skipped loudly without a database rather than falling back to something that runs everywhere.
  - **Three rows per store, not one, and the expected winner inserted last.** A one-row table returns for any probe, so *"query returned 1"* shows the SQL executes and says nothing about ranking — `test_all_three_graph_adapters_import` in test form (W30). The first draft still could not tell: with the winner inserted *first*, correct ranking for `decisions` was **identical to insertion order**, and only `communities` disambiguated, by luck. Inserting the winner last makes ranking and insertion order give different answers.
  - **The dimension is read from the database, not chosen.** These tables are shared — the name carries no dimension and only the `workspace` column separates callers — so a hardcoded `n` passes on an empty database and fails on any real one with `expected 1536 dimensions, not 64`.
  - **Controls run:** wrong expected winner → red; `delete` stubbed to a no-op in the adapter → red; both restored, suite re-verified.
- [x] **Delete the D-039 startup refusal** — `QUADRUPLE_UNSUPPORTED_VECTOR_STORES`, `QuadrupleUnsupported` and `assert_quadruple_supported` are gone from `weave_core/graph/storage/__init__.py`, with the call site and the `except` clause in `weave/server/app.py`. `tests/test_quadruple_backend_refusal.py` is deleted; `test_postgres_runs_quadruple_mode.py` replaces it and fails if any of the three symbols returns, or if any startup precondition refuses the pair again.
  - **Deleting it exposed a live defect it had been masking (W57).** `refuse_readably` caught `(InsecureSigningSecret, QuadrupleUnsupported, ValueError)`. `BusDeploymentMismatch` — the A7 refusal — is a `RuntimeError` and is **none of those**, so it has reached operators as a traceback since it was written. Nobody noticed because the one test driving that wrapper drove the quadruple refusal, which *was* listed by name. **One example proved the property for the example.**
  - Fixed, and the test rewritten as a property: `tests/test_startup_refuses_before_it_announces.py` now drives **every** precondition through `refuse_readably` and demands a `SystemExit`, plus a guard that fails when a precondition has no row. Controls: dropping `BusDeploymentMismatch` from the tuple → red; adding a precondition without a row → red.
- [x] `docs/CONSTRAINTS.md` — A4's quadruple sentence removed at **v8** (the plan said v6; A4 was amended twice in between, by D-046 and D-047), amendment row added, **`D-053` logged** — and both written *before* the refusal was deleted, in that order.
  - **The approval column reads `dsivov (pending)`, deliberately.** The plan chartered this change and the round-trip discharged its stated condition, but no human approved it in the session that made it, and the amendment table's own rule is *"no entry without a human's explicit approval."* Recording a name that was not given would make the audit trail the one document that lies. **This needs dsivov's sign-off to become final** (precedent: the four v2 rows).
- [ ] **[manager]** re-run `docker compose -f deploy/compose.yml up` and confirm a governed workspace on PostgreSQL, end to end. **Unblocked:** the refusal that stopped it is gone, and W56 removed the parse error in front of it.
  - **Before-state verified 2026-08-20, through the bundle rather than around it.** `docker compose -p weave-p9 up --build server postgres`: postgres **healthy**, server **Exited (1)** on the D-039 refusal — *"PGVectorStorage cannot run Weave's governance mode yet … `deploy/compose.yml` ships this pair as its default, and it has never worked."* **The published quick-start does not start today**, on its own defaults, which is the plainest statement of what P9 is worth. The refusal names the phase and offers two working alternatives, so the failure is honest — but it is still a failure at the front door.
  - **Also found on the way in: W56** — the file will not parse without `WEAVE_NEO4J_PASSWORD`, for a service behind a profile that is not being started.
...
- [x] **Contract check (R11)** — **A3** ✓ nothing here is a banned spelling; that is the point, and the new tests assert *content* rather than names · **A5** ✓ the examples model artifacts and their locators, never document bodies · **A11** ✓ no new library · **A13** ✓ prompt and harness are server-side LLM use only; nothing goes near a Claude Code process.
- [x] `scripts/measure_extraction.py` — fixed corpus, counts by type, and a leak check that **reads the example names out of the live prompt** rather than hard-coding the five we found; a fixed list would pass the moment the examples changed, which is this very phase. **Not run here: this container has no model credential** (A13), so the before/after numbers are unmeasured by me — `--names-only` and the comparison path work offline.
- [x] `weave_core/graph/prompt.py` — **three example blocks, not two.** The plan named `entity_extraction_examples` and `cg_entity_extraction_examples`; a third, `cg_entity_extraction_json_examples` inside the JSON-mode system prompt, carried the same sales conversation and the Barack Obama excerpt. All three now teach on RFC/ADR/CR/TASK/commit/review/insight artifacts with types from the ontology. The keyword examples (international trade, deforestation, poverty) were the same class and went too.
- [x] `tests/test_the_extractor_learns_from_software_artifacts.py` — 11 tests over **every** key containing "example", scoped to the whole file rather than the blocks, because the third block is exactly what a named list misses. Six controls, all firing. **A cost of the fix, made explicit:** examples drawn from the domain are *harder* to catch leaking — "AudioRival" in a software graph is unmistakable, "PostgreSQL" is what a real document produces — so the gate turns on copied identifiers (`RFC-014`, a sha, a run id) while the report lists every overlap.
- [x] **Closed by P15, with a model.** Both items were blocked on a credential, and re-extracting under the parent's *vocabulary* would have measured the wrong thing anyway. Measured on gpt-4o-mini over a real corpus: **not one node of the parent's fourteen types** (`Objection`, `Competitor`, `LossReason` — 0 of 76), so the leak is gone by construction rather than by cleanup.
- [x] `docs/` — **the before/after numbers are recorded where the claim is made** (R2/R6): D-050, D-052, W48 and W50 carry conformance **66.5% → 92.8%** and answerability **42.5% → 47.3%** across four measured arms, each with its run count and range. **One of the four measured worse and shipped as a finding rather than a fix.**
- [x] **Swept:** `weave/wizards/templates/*.json` and `weave/team/preset/seed.json` carry none of the parent's worked-example content. Checked rather than assumed.
...
## P15 · Extraction, answers and checks read the ontology → **M15**  *(CR-003 · D-050)*

> **Approved 2026-08-15.** Weave installs an ontology into a workspace as **signed governance** — the
> object types the whole answer surface is built on — and **three separate places never read it**, each
> keeping a list by hand instead. Full text:
> [WEAVE_EXTRACTION_TAXONOMY_CHANGE_REQUEST.md](WEAVE_EXTRACTION_TAXONOMY_CHANGE_REQUEST.md).
>
> ```
> hand-written list                should come from        consequence, measured
> CONTENT_FIELDS (12)   W39         what nodes carry        `description` dropped — 97% of nodes
> DEFAULT_ENTITY_TYPES (14) W40     the ontology            zero overlap; nothing extracted is answerable
> ARTIFACT_TYPES (10)   W42         the ontology (18)       8 types invisible to the rot check
> ```
>
> **This is why the Learnings and Features tabs are empty, and it is not the demo data.** A real publish
> produced 92 nodes — `artifact 23 · concept 31 · method 6 · data 4 · objection 2 · constraint 6 ·
> person 1 · content 5 · UNKNOWN 14` — **not one Weave type**. Of the demo workspace's 975 nodes, every
> one the pipeline extracted is typed in a vocabulary the answer surface does not look for.
>
> **The one that must not happen:** capturing the types at engine construction. The ontology is a signed,
> versioned artifact that changes **without a restart** (A8) — types read once would go stale the moment
> someone signs a new version, and *the wizard would again write what the runtime does not read*. That is
> the failure A8 exists to prevent, arriving from a third direction, and it would pass every test.

- [x] **Contract check (R11)** — **A5** ✓ extraction now produces the ontology's artifact types · **A8** ✓ read per run; a re-signed ontology changes the answer without a restart, verified live · **A9** ✓ one list, one place · **A11** ✓ no new library · **A3** ✓ the parent's domain vocabulary is gone from a real extraction (0 of 76). **A8** (**load-bearing** — read per run, or a signed change is not in force), **A9** (one list read in one place, so REST/UI and MCP cannot diverge), **A11** (no new library), **A3** (`LossReason`, `Objection`, `Competitor` are the parent's *domain* vocabulary — not a name-guard hit, the same **semantic** carry-over as D-041).
...
- [x] **[manager]** `scripts/seed_demo.py` — **18 of 18, measured on a fresh workspace**, 36 nodes carrying a locator. Four of the ten come through their real product routes; the other six had none, which is W43. Never seeded: `Task`, `PRD`, `RFC`, `Diagram`, `Module`, `Question`, `Worker`, `DevHost`, `Environment`, `IntegrationRun`.
- [x] `tests/test_seed_covers_the_ontology.py` — 15 tests, 3 controls, **including the ontology-versus-seed comparison**, so a type added to the ontology fails until the seed covers it. Otherwise the next type goes unseeded exactly as these ten did.
- [x] **[manager]** `guides/WEAVE_USER_GUIDE.html` — chapter 6 carries the ontology→extraction→questions chain as a diagram. **Needs one correction now** (below): the read side normalises, so an existing graph is no longer *punished* for the types it was built with.

**This completes P11's two open items.** Re-extracting the demo tenant and recording the before/after
numbers were blocked on a model credential and on this: re-extracting under the parent's vocabulary would
have measured the wrong thing.

**Gate (M15) — criterion 1 is a *pair*, and neither half may be read alone (D-052).** Ingest into a
workspace with the ontology installed, then measure **both**:
**(1a) conformance ≥ 75%** of *extracted* nodes carry a type the ontology declares, or `Other`; **and
(1b) answerability ≥ 40%** carry a type the ontology declares — `Other` excluded, because `Other` is
legal and **unanswerable**. **A change that raises 1a while lowering 1b fails the gate even if 1a
passes.** Measured over the 22-document corpus, **at least three runs a side**, invented nodes
excluded from both numerator and denominator; a difference counts only if the ranges are disjoint · **`ask_features` over a freshly ingested corpus returns
nodes the pipeline extracted, not only hand-created ones** — *this is the criterion that matters; it is the
original question* · sign a new ontology, ingest again, and the **new** types are used **without a restart**
· a workspace with no ontology falls back to the **preset's** types, not the parent's · `WEAVE_ENTITY_TYPES`
still overrides · `seed_demo.py` produces **18 of 18** types and a test asserts the two lists match ·
`check_locators.py` resolves a published artifact **with no `--working-dir` flag** · R2 before/after on one
corpus: node count, type histogram, and the answerable-node count.
...
### Gate re-run 2026-08-16, **on gpt-4o-mini** — six of eight pass

dsivov supplied a model (D-051), so the three unmeasurable criteria were measured. **W46 was found
and fixed between the two runs**; these are the numbers after it.

| # | criterion | verdict |
|---|---|---|
| 1 | every extracted node's type is one the ontology declares, or `Other` | **fail — 69% of what extraction actually produced.** 76 nodes, of which **15 are pipeline-invented** (9 relationship endpoints, 6 governance decision endpoints) and never extraction output. Of the 61 that were: 38 ontology + 4 `Other` = **42 legal**, 19 off-vocabulary (`concept` 18, `script` 1). **W47.** |
| 2 | `ask_features` returns nodes **the pipeline extracted** | **PASS.** `/ask/features` → **15 nodes, all extracted**, none hand-created. *dsivov's original question, answered.* |
| 3 | a re-signed ontology is used without a restart | **pass** |
| 4 | no ontology → the preset floor | **pass** |
| 5 | `WEAVE_ENTITY_TYPES` overrides, documented | **pass** |
| 6 | R2 before/after, reporting the answerable-node count | **pass, and this is the headline: answerable nodes went from `0` to `38 of 76`.** Before D-050, *every* extracted node was typed in a vocabulary the questions do not query. |
| 7 | seed produces 18 of 18 | **pass** |
| 8 | `check_locators` with no flag | **pass** — `resolved: 29 · dangling: 0` |

**The finding that changes what W46 means.** The prompt the model receives is correct — verified out
of the LLM response cache: *"One of `Role,Feature,ChangeRequest,…`, or `Other`"*. **gpt-4o-mini returns
`module`, `feature`, `changerequest` in lower case anyway.** So the writer half of W46 (stop
destroying the spelling) cannot fix this on its own — **the read-side `normalize_type` is the half
that is load-bearing**, and the developer's judgement that the writer fix is belt-and-braces turns out
to be right for a reason neither of us predicted: the code no longer lower-cases, and the model does.

**M15 stays open on criterion 1 alone.**
...
| W32 | **The published command for joining a machine to the fleet does not authenticate, and the flag that fixes it is undocumented.** `guides/first-fleet.md` said `python3 -m weave.devhost --server … --workspace …`, and P8 copied it. **Executed:** the daemon starts, confirms the seat (*"subscription auth confirmed; API/Bedrock/Vertex scrubbed"*) and then loops on `HTTP Error 401: Unauthorized — holding 0 container(s) and retrying`. It needs `--token`. **And `--token` and `--api-key` carry no help text at all** — they appear in the usage line and have blank descriptions, so `--help` does not tell a reader what the fix is either. The daemon's own behaviour is good: it retries rather than exiting, and says what it is holding. **The failure is that the two documents a new operator reads — the guide and `--help` — both omit the required argument.** Found by P8's execute-before-you-write rule; the guide now carries the invocation that ran, and the successful output. | Manager, executing P8 §8, 2026-08-14 | **With the next dev-host change.** Give both flags help text; consider `$WEAVE_TOKEN` as a fallback the way `--server` and `--workspace` already read the environment, since a token on a command line is a credential in shell history. |

| **W33** | **CRITICAL · The MCP surface is unauthenticated, and the tenant is chosen by a client-supplied header.** Found while writing the guide's *"how does a manager connect?"* chapter — dsivov asked where role setup was documented and it was not. Measured on a server with authentication **enabled and working**: `GET /weave/status` → **401**, `GET /weave/tasks` → **401**, and `POST /mcp` with **no credentials at all** → **200**, returning the full tool list, and **`tools/call` executes**: `get_manifest` returned the workspace's ontology and `ask_learnings` ran the answer surface. **And the workspace comes from the `WEAVE-WORKSPACE` header on that same unauthenticated request** — `demo` and `default` each answered with their own manifest. So anyone who can reach the port reads any workspace, and the guide tells operators to bind `0.0.0.0`. **Against A6 directly:** *"the principal it is enforced against is derived from the authenticated identity … No endpoint bypasses either half."* `app.mount("", mcp_app)` (`app.py:2123`) attaches the MCP app outside the dependency that guards every REST route. **W16 recorded half of this** — *"MCP carries no role"* — and the manifest confirms it (`"role": null`); nobody checked whether it carried an **identity**. It does not. **This is M2's Critical in a new place:** the tenant selected by an unauthenticated client-supplied field. | Manager, writing P8's role-connection chapter, 2026-08-14 | **Before anything else.** The guide cannot document connecting a role to a surface that has no roles and no identities, and the product cannot be exposed on a network until this is closed. Needs a decision on how an MCP session authenticates — the kit's `.mcp.json` carries no credential today **because none is required**, which is the shape of the fix as well as the defect. |

| **W34** | **CRITICAL · Workspace membership is enforced on MCP and on SSE, and not on ordinary REST.** Measured on current code with a real user: `carol` is a member of **`demo` only**. `GET /weave/status` with `WEAVE-WORKSPACE: demo` → **200** (correct); with **`WEAVE-WORKSPACE: default` → 200** — a workspace she is not a member of. The same header on `/mcp` → **403**. So **an authenticated user reads any workspace over REST by changing one header**, while the surface W33 just hardened refuses it. `User.may_access` has existed since **P1** and `tests/test_membership.py` asserts it **against the store**; `live.py` consults it for SSE and `app.py:477` for the MCP mount, and **no ordinary REST route has ever called it**. **A4 calls the workspace the tenant boundary and A14 makes membership explicit — on the product's main surface it is neither.** W33 was *unauthenticated* access; this is *authenticated cross-tenant* access, and it is the same shape as M2's Critical: the tenant taken from a client-supplied header without checking the principal may have it. **Found by the developer while closing W33** — they put the rule in `weave/server/utils.py` as one function so REST can adopt it unchanged, and wired only the MCP mount, flagging that enforcing it everywhere changes who can see what. | Developer during W33; measured by the manager, 2026-08-14 | **Immediately, and it is not only a repair** — turning it on removes access some caller may be relying on, so it wants a `D-NN` naming what breaks. The function already exists; the work is applying it to every workspace-scoped route and asserting it **per route**, not once. |

| W35 | **A refused request still creates the workspace it named.** `WorkspaceMiddleware` resolves the header by calling `pool.get_rag(workspace)` **before** the auth dependency runs, so admission precedes authentication. Verified after D-048: `carol`, a member of `demo` only, asks for `not_carols` and gets a correct **403** — and `weave_storage/not_carols/` **exists on disk afterwards**. No data leaks and nothing is readable, so it is much smaller than W34; but it is the same header doing something it should not, and an authenticated user can create unbounded directories by naming them. | Developer during W34; confirmed by the manager, 2026-08-14 | **When middleware ordering is next opened.** Moving admission after authentication is an ordering change with its own blast radius — every path that resolves a workspace runs through it — so it wants measuring the way D-048 was measured, not a quick reorder. |
| W36 | **One endpoint is public because nobody ever declared it either way.** `/workspace/backfill-script` carries **no auth dependency**: unauthenticated it answers (404 here, with a helpful message naming where to fetch the script instead). It serves fixed content and reads no workspace, so it is not a tenant question and is **probably intended** — the guide tells an operator to `curl … | python -`. **The defect is that it is undeclared.** W33 was exactly this shape: an endpoint outside the auth dependency that nobody had looked at. A public route should be public *on purpose*, in a list, with a test — otherwise the next one arrives the same way. | Developer during W34; confirmed by the manager, 2026-08-14 | **With the next auth change.** Declare the intentionally-public set and assert it, so the property is *"these routes and no others are reachable unauthenticated"* rather than *"this one seems fine"*. |

| W37 | **`scripts/measure_extraction.py` cannot run its measuring path — on any machine, with or without a credential.** It constructs `WeaveGraph(working_dir=…)` and **never supplies an embedding function**: `grep -c embedding` over the file returns **0**, and the storage layer refuses with *"embedding_func is required for vector storage"* before a single document is read. Run here with a working OpenAI credential and a live embedding server configured — it fails identically. So **P11's R2 half has never executed**, and the developer's report that it "needs whoever holds a credential" was the reasonable inference and not the cause. **The offline paths (`--names-only`, `--compare`) work**, which is exactly why this was not noticed: a harness whose cheap paths run reads as a harness that runs. Same family as `test_all_three_graph_adapters_import` and the store-only membership test — the part that was exercised sat next to the part that mattered. | Manager, running P11's gate, 2026-08-14 | **Before D-041's measurement can be claimed.** Wire the embedding backend from the same `WEAVE_EMBEDDING_*` variables the server reads, then take the before number against the pre-P11 prompt and the after number against the current one. **The before must come from the old prompt** — a baseline taken after the change is not a baseline. |

| **W38** | **CRITICAL · `weave docs publish` deletes the document it publishes.** Reproduced on a throwaway copy, so it is the behaviour and not an accident of my first run: `cp CONSTRAINTS.md probe.md` (12,616 bytes) → `weave docs publish --path probe.md` → *"published PROBE-1 → probe.md"* → **the file is gone.** I hit it first on the real thing: publishing `docs/WEAVE_RFC.html` removed it from the working tree (`git status` showed ` D`), and it was recoverable **only because it was committed**. An uncommitted artifact — which is exactly what a session has just authored when the kit tells it to publish — **is destroyed with no copy anywhere.** **This inverts CR-002's premise.** The CR, the guide's chapter 10 and its diagram all say *the repository stays the source of truth; Weave holds a reference, never a copy*. Instead the publish consumes the source and the locator is left pointing at a path that no longer exists — so the very next `check_locators` run would call it dangling, correctly. `publish_artifact` hands the **real path** to `pipeline_enqueue_file`; the deletion in that function's `finally` is guarded by a temp-prefix check that a repository file does not match, so the consumption happens elsewhere in the ingest path and needs finding rather than guessing. | Manager, running P14's gate, 2026-08-15 | **Before P14 can be called done, and before anyone runs the guide's chapter 10.** Publish must **copy** into the ingest path and never touch the source. Gate: publish a file, assert it still exists **and is byte-identical**; publish twice and assert the second is a no-op — which cannot even be tested today, because the first run removes the input. |

| **W39** | **The answer surface drops `description` — the one field 97% of nodes carry.** dsivov asked whether the thin Learnings and Features tabs are a demo-data problem or a design one. **Design, and it is measurable:** of **975 nodes** in the demo graph, **947 (97%) carry a description**, and `CONTENT_FIELDS` in `weave/model/answers.py` lists twelve fields — `title, status, summary, verdict, statement, sha, reviewer, confidence, text, asked_by, url, path` — **and `description` is not one of them.** So `/ask/features` returns `{id, type}` per node and nothing else: the Features tab shows six names because six names is all the answer contains. The content is in the graph and correct — `Feature P0 — Fork and rebrand` holds *"92k LOC copied from a proven engine and renamed, with a guard that fails the build on any surviving parent name"* — and never leaves it. **No renderer can fix this**; U3 fixed the layer above and this is the layer below. **`description` is the engine's universal field**: every extracted entity has one, so this is not a Features problem but a systematic loss across every answer. Learnings escape it only because `record_learning` happens to write `statement` too. | dsivov's question, measured by the manager, 2026-08-15 | **Before the guide claims the answer surface is useful.** Add `description` to `CONTENT_FIELDS` and to the label chain's fallbacks, then re-check what each of the four questions returns — the fix is one line and **the value of the four questions turns on it**. Worth asking at the same time what *else* the twelve omit; the list was written by hand and this is the first time anyone has compared it against what nodes actually carry. |

| **W40** | **Extraction categorises with the parent's taxonomy, not Weave's ontology — so almost nothing it ingests is the type the answers look for.** `DEFAULT_ENTITY_TYPES` (`weave_core/constants.py:29`) is **Person, Creature, Organization, Location, Event, Concept, Method, Content, Data, Artifact, NaturalObject, LossReason, Objection, Competitor**. Weave's own ontology — the one the product *signs into the workspace* as governance — is **Role, Feature, ChangeRequest, Task, ArchitectureDecisionRecord, Review, Insight, Question, Module, Commit, PRD, RFC**. **The overlap is zero.** `Creature`, `NaturalObject`, `LossReason`, `Objection` and `Competitor` are the sales-and-general-knowledge vocabulary the extraction examples used to teach (D-041), and while P11 replaced the *examples*, **the taxonomy they illustrated was never touched**. Measured on a real publish: 92 nodes came back as `artifact 23 · concept 31 · method 6 · data 4 · objection 2 · constraint 6 · person 1 · content 5 · UNKNOWN 14` — **not one Weave type.** This is the other half of dsivov's question: the answer surface seeds on `Feature`, `Review`, `Insight`, so it finds only the handful of nodes a human created by hand and none of the 947 the pipeline extracted. **D-041's deepest layer**: the examples were the illustration; this is the schema. | dsivov's question about the thin tabs, measured by the manager, 2026-08-15 | **Before the answer surface can be called useful, and with W39.** The workspace already *has* an authoritative type list — the installed ontology — and `preset.load_part("ontology")` is what P11's tests already assert against. Extraction should take its types from there rather than from a constant carried out of the parent. Expect this to change what every future ingest produces, so it wants a measurement and a `D-NN`, not a quiet edit. |
| W41 | **A real publish reports success and leaves no artifact node.** `weave docs publish --artifact PROBE-2 --type RFC` printed *"published PROBE-2 → probe.md @ working"* and exited 0; the workspace graph afterwards holds **92 nodes and zero `PROBE-2`**, and no node of type `RFC`. `check_locators` then reports **`resolved: 0 · dangling: 0`** — *"zero dangling"* out of nothing examined, which is the vacuous pass this project keeps finding and which the CR's acceptance criterion would have accepted. **Hypothesis, not yet proven:** the node is upserted and then lost, because `apipeline_process_enqueue_documents` rewrites the graph file after it — an ordering that a stubbed ingest cannot reproduce, which is why P14's tests pass. **Not diagnosed further deliberately** — the developer knows that code and I would be re-deriving it. | Manager, running P14's gate on a real model, 2026-08-15 | **Blocks P14's gate.** And the acceptance criterion needs strengthening either way: *"`check_locators` reports zero dangling"* must become *"reports the published artifact as **resolved**"*, because zero-of-zero passes today. |

| W42 | **`check_locators.py` ignores `WEAVE_WORKING_DIR`, so the rot check reports a clean bill from an empty directory.** `--working-dir` defaults to **`./weave_storage`** (`scripts/check_locators.py:199`) and reads no environment variable — while `weave/server/__init__.py` now holds the one `DEFAULT_WORKING_DIR` + `resolve_working_dir()` that D-048 introduced precisely so the CLI and the server could not disagree. **The script was not swept.** Measured: with `WEAVE_WORKING_DIR` exported and a published artifact present, the check reported `resolved: 0 · dangling: 0` — *"0 dangling locators"* from a directory it had never looked in. Named explicitly with `--working-dir`, the same run finds the node and classifies it correctly (`unregistered`, because that publish came from outside a registered repo). **This is W27's defect surviving in a script, and it produces the vacuous pass on CR-002's own acceptance criterion** — which is why that criterion is being changed to *"reports the published artifact as **resolved**"*. **Second observation, same file — and it did not survive measurement; corrected here rather than left standing.** I wrote that `ARTIFACT_TYPES` (a hand-written set of **10**) left 8 of the ontology's 18 types invisible to the rot check. **It did not.** Exactly **10** object types declare a `locator_*` property — `ArchitectureDecisionRecord, ChangeRequest, Commit, Diagram, Feature, Insight, PRD, RFC, Review, Task` — and the hand-written ten *were* those ten. The other eight (`DevHost`, `Environment`, `IntegrationRun`, `Module`, `PullRequest`, `Question`, `Role`, `Worker`) are correctly excluded: a `Worker` holds no document, and requiring a locator of one would report rot that does not exist. The list is now **derived** from the ontology regardless — a list that *cannot* drift beats one that happens to be right today — but **the defect in W42 was the working directory alone.** Found by the developer, who measured the claim instead of accepting it. | Manager, running P14's gate, 2026-08-15 | **With W39/W40 — they are one theme.** Adopt `resolve_working_dir()`, and take the artifact types from the installed ontology rather than a set maintained by hand. |

| W43 | **The question surfaces seed on types no product path creates — `/ask/features` on five of them.** Static sweep of every `entity_type` literal in `weave/` and `weave_core/`: the runtime creates `Commit`, `PullRequest`, `Review`, `Insight`, `Role`, `Worker`, `DevHost`, `Environment`, `IntegrationRun`. It creates **`Feature`, `Module`, `PRD`, `RFC`, `Diagram` and `Task` nowhere.** `answers.py:39` — `FEATURE_TYPES = ("Feature", "Module", "PRD", "RFC", "Diagram")` — so **every one of the five types `/ask/features` can return is a type nothing in the product produces**; the six `Feature` nodes in the demo exist because `seed_demo.py` creates them by hand, and `Module`, `PRD`, `RFC` and `Diagram` have never existed in any instance. **`Task` is worse, because a record does exist:** `coordinator.create_task` writes a `WeaveTask` to the record store and never upserts a graph node, while `CHANGE_TYPES` and `WHY_TYPES` both seed on `Task` — so `/ask/changes` walks a chain whose first link is missing and `/ask/why` cannot reach the task a decision was about. **This is the layer above W40**: even once extraction emits ontology types, it emits them only when a document happens to *describe* a feature or a module. **Nothing in the team's own workflow — creating a task, delivering a feature, publishing a PRD — produces the node the question looks for.** The only mechanism that types an artifact node is CR-002's `publish_artifact`, and it takes the type from its **caller**, unvalidated against the ontology. | Manager, seeding all 18 ontology types (P15), 2026-08-16 | **This is the honest answer to *"why are the tabs uninformative"*, and it is not the demo data.** Two separable pieces: (a) `create_task` should upsert a `Task` node with its locator — the record and the node are the same fact stored twice, and only one is answerable; (b) `publish_artifact` should refuse a type the installed ontology does not declare, which is the same *check-against-the-ontology* rule P15 is already applying in three places. |

| W44 | **`/graph/entity/create` silently discards the locator — so A5's mechanism is unavailable through the product's own entity-creation endpoint.** `weave_core/graph/query.py:955` builds `node_data` as a **hand-written dict of six fields** — `entity_id`, `entity_type`, `description`, `source_id`, `file_path`, `created_at` — and every other key the caller passed is dropped **without a warning**, including `locator_repo`, `locator_path`, `locator_rev` and `locator_anchor`. `/graph/entity/edit` preserves them; only `create` loses them. **Measured on a clean tenant:** seven `Task` nodes written through `edit` carry `locator_path='docs/WEAVE_WORK_PLAN.md'`; every `Feature`, `PRD` and `Module` written through `create` carries `locator_path=None`, and `/ask/changes` returns `locators resolved: 7` against `28` with none. **Every `Feature`, `ChangeRequest` and `ArchitectureDecisionRecord` node in every demo this project has ever built has had no locator** — and `seed_demo.py`'s own docstring opens *"Every locator in here resolves to a file that actually exists in this repository."* **That sentence has been false since the demo existed.** It is also a second, independent cause of the vacuous `check_locators` pass in W42: pointed at the right directory it still had almost nothing to resolve, because the nodes carry no locator to check. Inherited parent code — the endpoint's own docstring still illustrates with `Tesla` / `ORGANIZATION`. | Manager, seeding all 18 ontology types (P15), 2026-08-16 | **High, and the fourth instance of the phase's theme** — after `DEFAULT_ENTITY_TYPES`, `CONTENT_FIELDS` and `ARTIFACT_TYPES`, this is a hand-written field list in the **write** path rather than the read path. Same fix shape: carry the caller's fields through, or refuse the ones the ontology does not declare — **silently dropping them is the one option that must go**. Note the asymmetry that hid it: the field survives `edit`, so anything written twice looks fine. |

| W45 | **A per-role LLM binding inherits the *global* binding's host, so `EXTRACT_LLM_BINDING=openai` points OpenAI at the local ollama port.** `weave/server/config.py:448` — `_role_llm` falls back to `args.llm_binding_host`, which was resolved from `WEAVE_LLM_BINDING` (default `ollama` → `http://localhost:11434`). Measured: `EXTRACT_LLM_BINDING=openai EXTRACT_LLM_MODEL=gpt-4o-mini` yields `{'binding': 'openai', 'model': 'gpt-4o-mini', 'host': 'http://localhost:11434'}`. **The embedding path gets this right** — `WEAVE_EMBEDDING_BINDING=openai` resolves its own default host (`https://api.openai.com/v1`) — because it calls `get_default_host()` against its own binding. The role path never does. **The failure is a connection error to a port that is usually closed, so it reads as "the extraction model is down" rather than "the host belongs to a different provider"** — which is the expensive part: it points at the model, not at the configuration. Workaround is to set `EXTRACT_LLM_BINDING_HOST` explicitly, which nothing tells you to do. | Manager, configuring GPT-4o-mini for extraction (D-051), 2026-08-16 | **A per-role override that changes the provider must re-derive that provider's default host**, exactly as the embedding path does — `get_default_host(role_binding)` rather than the global host. Same family as W27/W42: a default resolved once for one caller and inherited by another that needed a different one. Also worth a startup warning when a role's binding differs from the global one and its host was not set. |

| W46 | **The extractor lower-cases every entity type, and the answer surface matches capitalised names exactly — so P15's fix does not reach the questions.** `weave_core/graph/operate.py:419` — `entity_type = entity_type.replace(" ", "").lower()` — inherited from the parent, where nothing matched on type. **Measured on gpt-4o-mini against a real document** (`WEAVE_EXTRACTION_TAXONOMY_CHANGE_REQUEST.md`, workspace `demo5`, ontology installed): 56 nodes, and **not one from the parent's vocabulary** — no `Objection`, no `Competitor`, no `LossReason`, which is D-050 working. But the types come back `changerequest`, `architecturedecisionrecord`, `prd`, `rfc`, `task`, `module`, `feature`, `insight`, `review`, `commit`, `pullrequest`, `question`, `worker`, `devhost`, `environment`, `integrationrun` — **every one of them lower-cased**, while `answers.py` declares `FEATURE_TYPES = ("Feature", "Module", "PRD", "RFC", "Diagram")` and matches on equality. Result: **`/ask/features`, `/ask/changes` and `/ask/learnings` all return 0 nodes from a graph that contains a correctly-typed node for nearly every type they ask about.** The `Role` row proves the split on its own — **4 capitalised** (seeded by bootstrap) and **1 lower-case** (extracted) sitting in the same graph as different types. **Nothing short of running a real model could have found this:** the resolver returns the right list, the prompt carries it, the extraction obeys it, and the product still answers nothing. | Manager, running the M15 gate on gpt-4o-mini (D-051), 2026-08-16 | **This is the P15 gate's criterion 2 failing, and it blocks M15.** Normalise once, in one place, and compare normalised — the fix must not be "capitalise in the extractor", because the surface would then depend on a model's casing. **Secondary, same run:** the model returned off-ontology types (`concept`, `document`, `function`, `person`, `module`) instead of the declared escape hatch, and used `Other` **not once** — worth a prompt check, but it is not what breaks the questions. |

| W47 | **45% of extracted nodes carry a type the ontology does not declare, and the declared escape hatch is barely used — criterion 1 of the M15 gate.** Measured on gpt-4o-mini, workspace `demo6`, two real CR documents, ontology installed, **compared the way the product compares** (through `normalize_type`): **76 nodes — 38 an ontology type (50%), 4 `Other` (5%), 34 neither (45%)**. The 34 are `concept` **18**, `UNKNOWN` **9**, `ENTITY` **6**, `script` **1**. Two different causes, and they need separating before anything is changed. **(a) `concept` and `script` are the model ignoring a correct instruction** — the prompt it received reads *"`entity_type`: One of `Role,Feature,ChangeRequest,…,IntegrationRun`, or `Other` if none apply"*, verified out of the LLM response cache, so this is prompt *adherence*, not a missing rule. `concept` is also a close relative of the parent's fourteen, which supports the developer's suspicion that the few-shot examples still teach a general-purpose taxonomy **by shape** even though their content is now software artifacts — measurable with the harness's `answerable_pct` before touching the prompt. **(b) `UNKNOWN` and `ENTITY` are ours, not the model's — and traced, they are not "defaults" at all: they are nodes the pipeline *invents*, and none of the 15 is extraction output.** Measured on `demo6`: **all 9 `UNKNOWN` nodes appear only as an edge endpoint**, and **8 of the 9 were never named as an entity by the model** — the extractor emitted a *relationship* mentioning them and `operate.py:1526` / `:2297` create the missing endpoint with a placeholder (*"Node doesn't exist — create new node"*). The 9th, `WEAVE_ENTITY_TYPES`, was extracted as `concept` in one chunk and auto-created as an endpoint in another, so it is the same mechanism plus a merge-order effect. **All 6 `ENTITY` nodes carry `source_id=emit_decision_trace`** — `p15mgr6`, `ontology:ontology`, `action:action`, `lifecycle:lifecycle`, `rule:rule`, `rbac:rbac` — governance ledger endpoints written by *bootstrap*, which never went near the extractor. **Same mechanism as W43.** **So criterion 1's denominator was wrong:** of 76 nodes, **15 were never extraction output**. Against the 61 that were: **42 legal (38 ontology + 4 `Other`) = 69%**, and 19 off-vocabulary — all of them cause (a). The gate should measure what extraction produced. | Manager, re-running the M15 gate after W46 (D-051), 2026-08-16 | **Do not fix this by widening the ontology.** The value of criterion 1 is that it measures the gap between what a workspace declares and what it gets. Split the two causes, fix (b), then measure (a) before and after a prompt change — R2, and the same discipline that governed P11. |

| W48 | **The extraction metric is not powered to detect a prompt change on this corpus — measured, and the experiment is the finding.** W47(a) said the few-shot examples still teach a general-purpose taxonomy: five hardcoded lists in `weave_core/graph/prompt.py` declare `Person` and `Concept`, which the ontology does not, and **no example anywhere demonstrates `Other`** (`"Other"` appears **0** times), while the instruction names it as the escape hatch. That diagnosis stands. **The fix does not.** Removing `Person`/`Concept` from the five lists and re-typing the two `Concept` example entities to `Other` was measured before and after on the same corpus, same model (gpt-4o-mini), same recipe — two real CR documents into a fresh bootstrapped workspace, counted through `normalize_type` with invented nodes excluded from **both** sides: **before {73.1%, 56.7%} · after {53.1%, 75.5%, 52.9%, 73.1%}** — means **64.9%** and **63.7%**. **The within-condition range is over 20 points and the between-condition difference is one point.** `concept` counts overlap the same way: before {12, 18}, after {15, 7, 10, 6}. **No effect is detectable, and more runs at this corpus size will not rescue it** — the variance is per-run model sampling on ~60 nodes. Reverted (R2: an unverified win does not ship), and the change also failed `test_the_declared_types_are_the_ontology_s`, whose `{person, concept}` carve-out is a deliberate decision that should be argued rather than quietly widened. | Manager, measuring W47(a) (D-051), 2026-08-17 | **Fix the measurement before the prompt.** A corpus of two documents cannot resolve a ten-point difference; a bigger corpus averages within a run and is the cheap lever (each run is ~3 minutes and pennies). Until then W47(a) is **diagnosed, not fixed** — and the diagnosis is worth keeping, because *"no example demonstrates the escape hatch the instruction names"* is true regardless of whether removing `Concept` helps. |

| W49 | **`measure_extraction.py --repeat` crashed on the second run and wrote nothing.** `--repeat 3` completed run 1 (22/22 document-passes), died 10 passes into run 2 with `RuntimeError: <asyncio.locks.Lock …> is bound to a different event loop`, and produced **no `--out` file at all** — the only signal was a stack trace in a log I happened to be tailing. The storage locks are created inside the first `asyncio.run()` loop and reused by the second. **`--repeat 1` is the default and works, so the flag was only broken at the values it exists for** — W20's family, and the third time this fortnight a defect has lived in the path nobody exercises. | Manager, running the powered W47(a) measurement, 2026-08-17 | **Fixed** by the developer: one `asyncio.run()` around the whole repeat loop, a test that *executes* two runs and asserts they shared a loop (a test asserting "measure is called twice" passes against the broken version), and partial results now reported with an `incomplete` flag rather than silently absent. Measurement completed meanwhile by running one process per run. |
...
- [x] **Closed-world half closed: `Ontology.has_object` compares normalised, and the examples were *not* allowed to make the question moot.** The prompt now teaches the ontology's spelling, so new extraction would have passed an exact check — and that is precisely the argument for fixing the check anyway: **a governance rule that holds only while a prompt behaves is an instruction, not a mechanism**, and this project has spent a fortnight on the difference. A graph built before P15.1, or a model that deviates, must not silently fail validation.
- [x] **It was three places, not one.** `has_object` was the shallow one. `validate_entity` resolved separately, and `LinkType.allows` / `validate._side_ok` compared the *endpoint types* exactly — so `DependsOn` resolved for `dependson` and then refused `changerequest → ChangeRequest`. **The second and third were found by a test written for the first**, which is the case for asserting the whole path rather than the entry point.
- [x] **Resolution is a scan, not a cached index** — `define_object` may be called after construction, and a cache that misses a later definition is the staleness this phase has already removed twice. Eighteen types make it free.
- [x] `tests/test_ontology_schema.py` + `tests/test_ontology_validate.py` — 12 tests, **three controls, all firing.** One of them did not: reverting `_side_ok` to an exact comparison left every test green, because nothing fed that path a spelling the extractor produces. **The control was silent, so the test was wrong, not the code** — fixed by exercising the validator rather than the helper.
...
| # | criterion | verdict |
|---|---|---|
| **1a** | conformance ≥ **75%** — extracted node carries an ontology type **or** `Other` | **pass — 86.6%** (22 docs, 3 runs, range 81–90) |
| **1b** | answerability ≥ **40%** — ontology type only, `Other` excluded | **pass — 46.8%** (range 44–52), the best of the four measured arms |
| 2 | `ask_features` returns nodes **the pipeline extracted** | **pass** — fresh workspace `gatews`, `/ask/features` 7 nodes (`feature` 6, `module` 1), `/ask/changes` 7, `/ask/learnings` 2, none hand-created |
| 3 | a re-signed ontology is used **without a restart** | **pass** — `demo4` resolves **19** types incl. `Postmortem`, `gatews` 18, on a server never restarted since signing |
| 4 | no ontology → the **preset** floor | **pass** — 18 preset types, none of the parent's fourteen |
| 5 | `WEAVE_ENTITY_TYPES` overrides, documented | **pass** — `--help` carries `--entity-types`; `PRD,RFC` and `["PRD","RFC"]` both parse |
| 6 | R2 before/after on one corpus, answerable-node count reported | **pass** — conformance **66.5% → 86.6%** (disjoint), answerability **42.5% → 46.8%**, and the number that started this: answerable extracted nodes **0 → 38 of 76** |
| 7 | `seed_demo.py` produces **18 of 18** | **pass** — fresh workspace `gseed`, 18 of 18, 36 nodes carrying a locator; 15 tests, 3 controls |
| 8 | `check_locators` resolves a published artifact **with no flag** | **pass** — `resolved: 29 · dangling: 0`, decomposing exactly |

**Suite:** 1378 passed, 15 skipped in the declared conda env — every skip an unconfigured backend or the
similarity model, none a P15 test. Name-guard clean.

**What the pair bought, stated plainly:** under the old wording criterion 1 would read **100%** on the
two-document workspace and the milestone would have passed at the very moment answerability was worst.
...
| W56 | **`deploy/compose.yml` cannot be parsed without a Neo4j password, even though Neo4j is behind a profile and A4 calls it optional and experimental.** `docker compose -p weave-p9 -f deploy/compose.yml up server postgres` fails before starting anything: *"error while interpolating services.neo4j.environment.NEO4J_AUTH: required variable WEAVE_NEO4J_PASSWORD is missing a value"*. The service **is** correctly guarded — `profiles: ["neo4j"]` at line 145 — so it is excluded from `up`; but compose interpolates **every** service's environment at parse time regardless of profile. **A profile makes a service optional to *run*, not optional to *configure*.** So an operator following the **PostgreSQL** path — the one the contract calls production — is blocked by a credential for a backend they deliberately did not enable, with an error naming a service they never asked for. | Manager, running P9's end-to-end half, 2026-08-20 | **Fixed 2026-08-20** — `:-neo4j-not-configured` instead of `:?`. **The `:?` form is right for `WEAVE_TOKEN_SECRET` and `WEAVE_POSTGRES_PASSWORD` and wrong here**, and that distinction is the fix rather than the syntax: *require what the deployment uses, not what the file mentions*. A placeholder is safe precisely because the service does not start without `--profile neo4j`, at which point anyone actually using it sets a real one. **Verified with both controls:** with no `WEAVE_NEO4J_PASSWORD` at all, `config --services` now lists `postgres` and `server`, and `up` reaches the **D-039 refusal** — the real blocker, the one P9 removes — instead of a parse error naming a service nobody asked for. Unset `WEAVE_TOKEN_SECRET` and it still refuses; unset `WEAVE_POSTGRES_PASSWORD` and it still refuses. **Left uncommitted in `deploy/compose.yml`**, which also carries the developer's uncommitted P12 build block — staging that file would sweep their work into my commit, which is the mistake I have already made twice. |
| W57 | **The A7 refusal has reached operators as a traceback since the day it was written, and a passing test said otherwise.** `refuse_readably` turns a startup refusal into a one-line `SystemExit` instead of eleven frames — that is the whole of W19. It caught `(InsecureSigningSecret, QuadrupleUnsupported, ValueError)`. **`BusDeploymentMismatch` is a `RuntimeError` and is none of the three**, so the event-bus refusal never went through the wrapper. It was invisible because the *outcome* is identical either way — the server refuses to start regardless; only the presentation degrades. And `test_a_refusal_exits_with_the_message_and_no_traceback` passed throughout, because the single configuration it drove was the quadruple refusal, which was listed **by name**. **One example proved the property for the example** — the same shape as the harness's `--repeat` proving isolation at n=1, and as `test_all_three_graph_adapters_import` reading as coverage. Found only because D-053 deleted the one refusal that worked. | Developer, deleting the D-039 refusal in P9, 2026-08-20 | **Fixed in P9.** `BusDeploymentMismatch` added to the tuple, and the test rewritten as a **property**: every precondition is driven through `refuse_readably` and must yield a `SystemExit` carrying its reason, with a second guard that fails when a precondition has no row in that list. Controls run both ways. **The general lesson, and it is the fourth sighting: a guard driven by one instance of a class certifies the instance.** Ask what the second one is — here there were three refusals and the wrapper covered two. |
| W58 | **The event-bus refusal tells a bundle operator to `export WEAVE_EVENT_BUS=postgres`, and `deploy/compose.yml` has no such variable** — W20's mechanism, in the refusal W20 never looked at. Found by widening `test_refusal_advice_is_reachable.py` from the (now deleted) quadruple refusal to **every** startup refusal, which is what a test of a property about refusals should always have done. **It is not a live dead end today**, and only for a reason worth writing down: that refusal cannot fire in the bundle, because it needs more than one worker and `WEAVE_WORKERS` is pinned to 1 (A7, D-019). So the advice is unreachable *and* so is the message offering it. | Developer, widening the W20 property in P9, 2026-08-20 | **Not fixed — declared, with its premise asserted.** `DECLARED_UNREACHABLE` in that test exempts `WEAVE_EVENT_BUS` and states the reason, and a companion test fails if `WEAVE_WORKERS` ever becomes overridable — at which point the refusal becomes reachable with advice the bundle still cannot take. **Left for the manager**, because the fix is one line in `deploy/compose.yml` (`${WEAVE_EVENT_BUS:-inprocess}`) and that file is currently carrying both their W56 fix and my uncommitted P12 build block; editing it now is the sweep-someone-else's-work mistake this project has already made twice. |

### M9 gate run 2026-08-21 — **passes, through the bundle**

**The published quick-start now starts.** Same command that produced `server Exited (1)` yesterday:

| | before (2026-08-20) | after |
|---|---|---|
| `docker compose up server postgres` | server **Exited (1)**, D-039 refusal | **postgres healthy · server healthy** |
| tables matching `%decision%` / `%communit%` | **0** | `weave_vdb_decisions_…1536d`, `weave_vdb_communities_…1536d` |
| governance bootstrap on PostgreSQL | never reached | **five layers signed**, AGE graph `m9_chunk_entity_relation` created per workspace |
| the write the refusal predicted would fail | — | **recorded, and in the store**: `m9 · A4 → T-M9 · D-053` |

`weave_vdb_decisions` holds **6 rows**, workspace-scoped, five from the bootstrap's own signing chain
and one from a governed `POST /weave/decisions`. **0 index-failure warnings.**

**Two things said plainly rather than buried.** `weave_vdb_communities` is **empty** — communities are
built by clustering during ingestion, and this run ingested nothing, so an empty table is the correct
state and not evidence of anything. **And the first attempt at this gate was mine, misconfigured:** I
ran the bundle with no embedding credential, every vector write failed, and the API still answered
`PASS`. That is W59.

| W59 | **A failed index write is invisible to the caller — `PASS` with nothing indexed.** `_index_decision` catches and logs at **warning**, deliberately: *"a failed index write must not undo the graph edge, which is the source of truth"*. That reasoning is right and the outcome is still wrong for an operator: I ran the bundle without an embedding credential, **every** decision write failed, and `POST /weave/decisions` returned `{"outcome":"PASS"}` each time. The graph edge existed; the precedent index was empty; nothing in the response said so. **A first-run misconfiguration therefore yields a silently degraded install** — the governance surface works, the precedent search is empty, and the only signal is a warning in a container log. | Manager, running the M9 gate, 2026-08-21 | **Not "raise instead"** — the design decision to keep the edge is correct. Surface it: a counter or `indexed: false` in the response, and `reindex_decisions()` already exists as the repair. The sharper point is that **`/health` reports the embedding binding as configured while every embedding call is failing** — a health check that cannot see the credential it needs is the same shape as W45. |

### Onboarding a real project — `bestbay_helper`, 2026-08-22

Ran the full sequence against a freshly `git init`-ed project (`0a4f276`) on the **production path**
(PostgreSQL, quadruple, OpenAI backend). It completes, and four things snag on the way.

| W60 | **`weave project register --repo <path>` and the locator's repo name disagree, so nothing an operator publishes ever resolves.** Registered `--repo /storage/Work/bestbay_helper`; `publish_artifact` derives `locator_repo` from the directory **basename** (`weave/model/artifacts.py:105` — `repo or repo_root.name`), so every artifact is stamped `bestbay_helper`. `check_locators` then reports **`unregistered: 3 — 'bestbay_helper' is not registered in this workspace`**, and `resolved: 0`. **A5's whole claim — every answer resolves to a real document — fails end to end on a real project, and nothing in the publish output warns of it.** | Manager, onboarding `bestbay_helper`, 2026-08-22 | **The blocking one.** Either `project register` should store the basename as the repo *name* alongside the path, or `publish_artifact` should resolve the registered project for the workspace rather than guessing from the filesystem. Guessing from the filesystem is what makes the two disagree. A test that publishes and then resolves — rather than publishes and checks the node exists — would have caught it. **Fixed 2026-08-22** — `weave project register` now writes the `ProjectLayout` too, deriving its name exactly as `publish_artifact` derives the locator's repo, and printing it. Measured on `bestbay_helper`: `resolved: 0 · unregistered: 3` → **`resolved: 2 · unregistered: 0`**. |

| W61 | **The bundled server has no mount for the repository, and `project register` accepts a path it cannot see without a word.** `deploy/compose.yml` mounts only `weave-data:/data`. Registering `--repo /storage/Work/bestbay_helper` from inside the container **succeeded and printed the summary**, for a path where `ls` returns *No such file or directory*. `weave docs publish` *does* catch it loudly — *"no such file … Publish the artifact that exists"* — so the failure surfaces one step later, in a different command, having already written the project record. | Manager, onboarding `bestbay_helper`, 2026-08-22 | Two halves: give the compose server a documented bind-mount for the working repository, and have `project register` **stat the path** when it is a local path (a clone URL is a different case and should stay unchecked). **Registering an unreachable repo is not a state worth keeping.** **Fixed 2026-08-22** — an **absolute** path that does not exist is refused at registration, exit 2. Narrowed to absolute paths after `test_setting_one_field_does_not_reset_the_others` caught the first version rejecting a bare repository *name*: a name is not a path, and a relative one may be resolved from a dev host's own checkout. **The test was right and the fix was wrong** — worth recording, because the instinct was to widen the test. |

| W62 | **The CLI is a second engine, not a client of the server — so a shell with a partial environment silently gets a different answer.** `weave docs publish` builds its own engine in-process, and `weave init` writes only the signing secret and two feature flags into `weave.env`. Model and storage configuration are **not** in that file. Measured, on the same machine, seconds apart: with the model vars absent, publish failed four times with *`model "mistral-nemo:latest" not found`*; with them exported, all three documents published. `scripts/check_locators.py` behaves the same way — without the storage vars it read a **file-based** graph and reported `resolved: 0 · dangling: 0` for a PostgreSQL deployment holding three artifacts. **W42 again, one layer up: the tool looked in the right directory and the wrong backend.** | Manager, onboarding `bestbay_helper`, 2026-08-22 | `weave init` should write the full runtime configuration it knows about, or these commands should refuse when their backend does not match the server's. **A clean bill from a different backend is the failure mode this project has now hit three times.** **Fixed 2026-08-22** — the server records its effective backend in `<working-dir>/runtime.json` (no secrets), and both `weave docs publish` and `check_locators` refuse when their own configuration differs, naming each difference. Reproduced from an empty environment: the rot check now says *"the server that last ran here uses PGGraphStorage, but this shell is configured for NetworkXStorage"* instead of printing `resolved: 0 · dangling: 0`. **Compared before the CLI's own overrides**, so the guard cannot fire on a difference it created — `product_engine` forces `use_quadruple`, and comparing afterwards would have taught operators to ignore it. |

| W63 | **`weave up` logs a bind failure and keeps running.** Port 9801 was held by an older server; `weave up` logged `[Errno 98] error while attempting to bind on address ('0.0.0.0', 9801)` and **did not exit**. Every subsequent request went to the *other* server, which answered healthily from a different working directory — so users created by the CLI "did not exist", and I spent four commands diagnosing a store mismatch that was a port collision. **A server that cannot serve should not stay up**, and the health endpoint reporting someone else's working directory is what made it look like a data problem. | Manager, onboarding `bestbay_helper`, 2026-08-22 | Exit non-zero on bind failure. Cheap, and it converts a confusing wrong-answer into an obvious one. **Note the shape:** the process was alive, the port answered, the reply was correct — for a different server. Nothing was wrong except *which* server. **Fixed 2026-08-22** — `weave up` probes the port before uvicorn and exits 1 with the reason and the fix. The probe can lose a race with uvicorn's own bind; losing it restores today's behaviour and never worse, which is stated in the code. |
