...
| W9 | **`ShellGit.test_cmd` defaults to `["python", "-m", "pytest", "-q"]`** (`weave/team/worker.py:341`). Debian/Ubuntu and this dev container ship `python3` with **no `python`**, so a dev agent on such a host fails every task at the test step with `FileNotFoundError` — and the loop reads that as "tests failed", records a learning, and moves on. A misconfiguration that looks like failing code. Configurable via `--test-cmd`, and correct inside the dev-agent image, so not urgent. | P5.3, hit while writing `test_pause_between_steps.py` | **P6**, where the dev-agent image and the published onboarding steps are the deliverable. Carried code — not fixed mid-P5 as unreviewed scope. |
