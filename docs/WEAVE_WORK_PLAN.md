...
- [ ] `tests/test_workspace_header.py` `[new]` — the middleware honours the documented header, and the lowercase trap is pinned by assertion rather than by comment.
- [ ] `tests/test_workspace_isolation.py` `[new]` — **two workspaces, written through the real store, see different data over HTTP.** M1's gate said "sees only granted workspaces" and passed because it was verified at the token/membership layer, never at the data layer; that hole is why this survived two reviews. This test must fail against the pre-fix middleware.
