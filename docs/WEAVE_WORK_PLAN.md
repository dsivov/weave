...
- [ ] **Open the pages in a browser before M7 is called.** Everything built in P7 so far is verified by `tsc`, `eslint` and a real bundle — **and has never been rendered**. *It compiles* is not *it works*, and this phase named that gap as its top risk. The navigation restructure and the new pages are unexercised at runtime. If `AnswerView` shows `(untitled)` against real data, the fix is its `title → name → entity_name → id` guess list, not the endpoint.
