...
- **Version:** v8  ·  **Agreed:** 2026-08-20 by dsivov  ·  **Status:** **in force**
- **Sources:** [WEAVE_RFC.html](WEAVE_RFC.html) · [WEAVE_DRP.md](WEAVE_DRP.md) · [WEAVE_ARCHITECTURE.html](WEAVE_ARCHITECTURE.html) · decisions D-002…D-053 in [DECISIONS.md](DECISIONS.md)
...
| A4 | State | Exactly three storage paths: file-based, PostgreSQL, Neo4j. No fourth backend. **All persistence goes through the `RecordStore` / `GraphStore` ports; no module constructs a database client outside its own adapter.** The three are not interchangeable, and the contract says which is which: **PostgreSQL is the multi-workspace production path**; file-based is single-operator only (whole-file read-modify-write); **the Neo4j path is experimental and supported for a single workspace only** — Community Edition cannot give a workspace its own database, so a second workspace on that path must be refused, not merely documented. **PostgreSQL is the multi-workspace production path, and the whole of it runs** — records *and* graph. `PGGraphStorage` needs **both** `age` and `vector` in one database, which no published image provides, so the bundle ships `deploy/postgres.Dockerfile` and runs against that (D-047). **PostgreSQL runs quadruple mode too**: `decisions` and `communities` are ordinary tables in that adapter, created and queried through the same `PGVectorStorage` as the other three (D-053). | Each path kept must be gated at every milestone (D-007), and a direct client bypasses the abstraction that makes three paths one codebase (D-020). Three paths listed as equals read as equally deployable; the workspace is the tenant boundary this system rests on, and only one of the three enforces it at the storage layer (D-029). |
...
| 2026-08-11 | v4 | A4 | was: three storage paths presented as equals → now: **the paths are ranked** — PostgreSQL is the multi-workspace production path, file-based is single-operator, and **Neo4j is experimental and single-workspace, with a second workspace refused** | dsivov | D-029 |
| 2026-08-13 | v5 | A4 | was: PostgreSQL is the multi-workspace production path, unqualified → now: **and it cannot yet run quadruple mode**, which is refused at startup rather than discovered at first use | dsivov | D-039 |
| 2026-08-14 | v6 | A4 | was: PostgreSQL is *the multi-workspace production path* → now: **the multi-workspace path for records, with its graph half not yet deployable** — `PGGraphStorage` needs `age` **and** `vector` in one image, no published image has both, and it has never run | dsivov | D-046 |
| 2026-08-14 | v7 | A4 | was: the graph half **not yet deployable** → now: **the whole path runs**, against a database image the bundle ships — earned by a graph round-trip on live PostgreSQL and a healthy server raised by the published steps | dsivov | D-047 |
| 2026-08-20 | v8 | A4 | was: PostgreSQL **cannot yet run quadruple mode**, the pair refused at startup → now: **it runs it** — `decisions` and `communities` are tables in the adapter, earned by a discriminating round-trip on live PostgreSQL (upsert 3, ranked retrieval, targeted delete) on both stores | dsivov | D-053 |
