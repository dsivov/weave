...
- [ ] `tests/test_ui_has_no_private_answer_path.py` — **corrected 2026-08-13.** The original name promised a UI/API node-set comparison that a Python test cannot make; the alternative, REST/MCP parity, is already covered by `test_mcp_rest_parity.py` and would have read as if it checked the UI. A9's real requirement is that the UI has no private data path — asserted by sweeping `weave-ui/src` for any call answering these questions other than the four `/ask` endpoints — plus a `bun test` that `AnswerView` drops no node the API returns.
