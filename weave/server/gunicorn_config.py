...
import os
import logging
from weave_core.store.locks import finalize_share_data
from weave_core.utils import setup_logger, get_env_value
...
errorlog = os.getenv("WEAVE_ERROR_LOG", log_file_path)  # Default write to weave_core.log
accesslog = os.getenv("WEAVE_ACCESS_LOG", log_file_path)  # Default write to weave_core.log
...
def on_starting(server):
    ...
    # Log the location of the WeaveEngine log file
    print(f"WeaveEngine log file: {log_file_path}\n")
...
def post_fork(server, worker):
    ...
    setup_logger("weave_core", log_level, add_filter=True, log_file_path=log_file_path)

    # Set up weave_core submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith("weave_core."):
